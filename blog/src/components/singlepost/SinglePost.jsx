import React from 'react'
import './SinglePost.css'
import musicLife from '../../Images/m2.jpg'

export default function SinglePost() {
  return (
    <div className='singlePost'>
        <div className="singlePostWrap">
            <img src={musicLife} alt="" className="singlePostImg" />
            <h1 className="singlePostTitle">Lorem ipsum dolor cxcxc cxcx cxcc sit
                <div className="singlePostEdit">
                 <i className="singlePostIcon far fa-edit"></i>
                 <i className="singlePostIcon far fa-trash-alt"></i>
                </div>
            </h1>

            <div className="singlePostInfo">
                <span className='singlePostAuthor'>Author : <b>Nishshanka</b></span>
                <span className='singlePostAuthor'>3 hours ago</span>
            </div>

            <p>Lorem ipsum dolor sit amet consectetur a
                dipisicing elit. Deserunt voluptatum tene
                tur doloremque, 
                labore esse temporibus quis quisqua
                m ea porro quidem laborum facilis eius odit 
                suscipit. Natus iusto non ratione sint?Lorem ipsum dolor sit amet consectetur a
                dipisicing elit. Deserunt voluptatum tene
                tur doloremque, 
                labore esse temporibus quis quisqua
                m ea porro quidem laborum facilis eius odit 
                suscipit. Natus iusto non ratione sint?Lorem ipsum dolor sit amet consectetur a
                dipisicing elit. Deserunt voluptatum tene
                tur doloremque, 
                labore esse temporibus quis quisqua
                m ea porro quidem laborum facilis eius odit 
                suscipit. Natus iusto non ratione sint?Lorem ipsum dolor sit amet consectetur a
                dipisicing elit. Deserunt voluptatum tene
                tur doloremque, 
                labore esse temporibus quis quisqua
                m ea porro quidem laborum facilis eius odit 
                suscipit. Natus iusto non ratione sint?Lorem ipsum dolor sit amet consectetur a
                dipisicing elit. Deserunt voluptatum tene
                tur doloremque, 
                labore esse temporibus quis quisqua
                m ea porro quidem laborum facilis eius odit 
                suscipit. Natus iusto non ratione sint?</p>


            
        </div>
    </div>
  )
}
